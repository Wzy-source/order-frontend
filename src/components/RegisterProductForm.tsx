    ...
    return (
        <form onSubmit={handleSubmit} style={styles.form}>
            ...
            <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} required disabled={!publicKey || isLoading}/>
            </div>
            <div>
                <label>Price (USDC):</label>
                <input type="number" step="0.000001" value={priceDecimal} onChange={(e) => setPriceDecimal(e.target.value)} required disabled={!publicKey || isLoading}/>
            </div>
            <div>
                <label>Image URL (Optional):</label>
                <input type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} disabled={!publicKey || isLoading}/>
            </div>
...
const styles = {
    form: { border: '1px solid #ccc', padding: '15px', marginBottom: '20px' },
} as const;
